    ...
    <main>
        ...
        >
            <bk-form ref="deleteForm" class="delete-form" :label-width="0" :model="deleteObj.formData">
                ...
                <p class="dialog-tip">{{$t('store.3、开发插件时，将遵循')}}<a href="https://iwiki.woa.com/x/ZAbl" class="text-link" target="_blank">{{$t('store.插件开发规范')}}</a>{{$t('store.，合理利用平台提供的公共资源。标准化错误输出，方便用户遇到问题时根据指引解决问题')}}</p>
