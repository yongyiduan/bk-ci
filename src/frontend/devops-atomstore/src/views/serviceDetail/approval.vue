        ...
        methods: {
            confirmApprove () {
                this.$refs.validateForm.validate().then((validator) => {
                    ...
                    this.$store.dispatch('store/approval', { type: 'SERVICE', code: this.serviceCode, approveId, approveMsg, approveStatus }).then((res) => {
                        if (res) {
            ...
            requestApproveList () {
                this.isLoading = true
                const data = Object.assign({}, this.pagination, { type: 'SERVICE', code: this.serviceCode })
