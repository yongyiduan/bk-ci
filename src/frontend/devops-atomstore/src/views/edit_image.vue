    ...
    <article class="edit-image-home">
        ...
        <main v-bkloading="{ isLoading }" class="edit-content">
            <bk-form ref="imageForm" class="edit-image" label-width="150" :model="form" v-show="!isLoading">
                ...
                <bk-form-item v-show="form.imageSourceType === 'THIRD'" :label="$t('store.源镜像Tag')" property="imageTag" :desc="$t('store.不建议使用可变功能的Tag（如latest），避免镜像变更导致关联流水线不能正常执行')" :required="true" :rules="[requireRule, latestRule]" ref="imageTag">
